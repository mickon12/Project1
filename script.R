...
US_UK <- subset(database, Country  == "UK" | Country  == "US")
...
ggplot(NL, aes(x = Senior.Management)) +
  ...
  theme_minimal()
str(database)
results <- apply(NL_UK[, c(3:9)], 2, function(x) {# Perform Wilcoxon test for each variable 'x' grouped by 'Country'
  test_result <- wilcox.test(x ~ NL_UK$Country)
  return(test_result)  
})
print(results)
results1 <- apply(NL_US[, c(3:9)], 2, function(x) {# Perform Wilcoxon test for each variable 'x' grouped by 'Country'
  test_result <- wilcox.test(x ~ NL_US$Country)
  return(test_result)  
})
print(results1)
print(results)
results2 <- apply(US_UK[, c(3:9)], 2, function(x) {# Perform Wilcoxon test for each variable 'x' grouped by 'Country'
  test_result <- wilcox.test(x ~ US_UK$Country)
  return(test_result)  
})
print(results2)
